...
def under_sample(matrix_X, matrix_Y, label, under_samp=30):
    ...
    Y_under_samp = list(
        np.split(random.sample(Y_under_samp_idx, len(Y_under_samp_idx)), [int(len(Y_under_samp_idx) / under_samp)])[1])
...
if __name__ == '__main__':
    ...
    label_column_strings = ["eyem", "chew", "shiv", "elpp", "musc", "null"]
    parser = argparse.ArgumentParser()
    parser.add_argument('--index', default=3, type=int)
    parser.add_argument('--reruns', default=3, type=int)
    parser.add_argument('--subsample', default=1, type=float)
    parser.add_argument('--label', default=5, type=int)  #delete for ex4?
    ...
    try:
        ...
        X_train_mini, Y_train_mini = under_sample(X_train_mini, Y_train_mini,
                                                  label=args.label, under_samp=1 / args.subsample)
        ...
        for run_nr in [args.reruns]:
            print("\n\nrunning run : %i\n\n" % run_nr)
            for alg in [select_alg[args.index]]:
                tic_mod = time.time()
                print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n",
                      "running on %s run %i" % (alg.name + '.one_V_all', run_nr),
                      "\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
                key_name = alg.name + ('_run%i' % run_nr)
                clf_method = one_vs_rest(str(key_name + '.one_V_all'), estimator=alg, n_jobs=1)
                estim_one_vs_rest[key_name + '.one_V_all'] = HyperoptEstimator(classifier=clf_method, loss_fn=myloss, n_jobs=n_job)
                estim_one_vs_rest[key_name + '.one_V_all'].fit(X_model_mini, Y_model_mini[:, select_classes],
                                                valid_size=val_dist, cv_shuffle=False)
                toc_mod = time.time()
                print("\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n",
                ...
                print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n",
                      "scoring on %s" % (key_name + '.one_V_all'),
                      "\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
                tic_score = time.time()
                model = estim_one_vs_rest[key_name + '.one_V_all']
                model_predict = model.predict(X_test_mini)
                algo_scoring[key_name + '_wF1'] = myloss(Y_test_mini[:, select_classes], model_predict)
                algo_scoring[key_name + '_acc'] = 1 - model.score(X_test_mini, Y_test_mini[:, select_classes])
                algo_scoring[key_name + '_sens'] = recall_score(Y_test_mini[:, select_classes], model_predict,
                                                                 average='weighted')

                # np.save(r"tmp/scores_night_2021_03_22.npy", algo_scoring)
                algo_scoring[key_name + '_report'] = classification_report(Y_test_mini[:, select_classes],
                                                                            model_predict,
                                                                            target_names=list(label_column.keys()),
                                                                            zero_division=0, output_dict=True)
                report_catch = algo_scoring[key_name + '_report']
                ## TODO: set it to loop mode later in code --- way more pretty
                lars_table["model names"].append(
                    alg.name.split("_")[-1] + '_%i_subsample_%.2f' % (run_nr, args.subsample))
                lars_table["wF1"].append(report_catch["weighted avg"]["f1-score"])
                lars_table["acc"].append(algo_scoring[key_name + '_acc'])
                lars_table['balanced acc'].append(report_catch["macro avg"]["recall"])
                lars_table["sens"].append(algo_scoring[key_name + '_sens'])
                ...
                algo_scoring[key_name + '_sens-eyem'] = report_catch[list(label_column.keys())[0]]["recall"]
                algo_scoring[key_name + '_sens-chew'] = report_catch[list(label_column.keys())[1]]["recall"]
                algo_scoring[key_name + '_sens-shiv'] = report_catch[list(label_column.keys())[2]]["recall"]
                algo_scoring[key_name + '_sens-elpp'] = report_catch[list(label_column.keys())[3]]["recall"]
                algo_scoring[key_name + '_sens-musc'] = report_catch[list(label_column.keys())[4]]["recall"]
                algo_scoring[key_name + '_sens-null'] = report_catch[list(label_column.keys())[5]]["recall"]

                algo_scoring[key_name + '_acc-eyem'] = accuracy_score(Y_test_mini[:, 0], model_predict[:, 0])
                algo_scoring[key_name + '_acc-chew'] = accuracy_score(Y_test_mini[:, 1], model_predict[:, 1])
                algo_scoring[key_name + '_acc-shiv'] = accuracy_score(Y_test_mini[:, 2], model_predict[:, 2])
                algo_scoring[key_name + '_acc-elpp'] = accuracy_score(Y_test_mini[:, 3], model_predict[:, 3])
                algo_scoring[key_name + '_acc-musc'] = accuracy_score(Y_test_mini[:, 4], model_predict[:, 4])
                algo_scoring[key_name + '_acc-null'] = accuracy_score(Y_test_mini[:, 5], model_predict[:, 5])
                lars_table['acc-eyem'].append(algo_scoring[key_name + '_acc-eyem'])
                lars_table['acc-chew'].append(algo_scoring[key_name + '_acc-chew'])
                lars_table['acc-shiv'].append(algo_scoring[key_name + '_acc-shiv'])
                lars_table['acc-elpp'].append(algo_scoring[key_name + '_acc-elpp'])
                lars_table['acc-musc'].append(algo_scoring[key_name + '_acc-musc'])
                lars_table['acc-null'].append(algo_scoring[key_name + '_acc-null'])
        ...
        performance_tall = pd.DataFrame.from_dict(lars_table, orient='index',
                                                  columns=lars_table["model names"]).iloc[1:]

        performance_wide.to_csv(save_score_path + r'/ex4_table_%i_runs%i_subsample_%.2f.csv' %
                                (args.index, args.reruns, args.subsample),
                                encoding='utf-8-sig')
